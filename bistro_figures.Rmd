...
# matches
provedit_exact_matches <- read_csv("output/provedit/exact_matches.csv") %>%
  separate(human_id, sep = "-", into = c("proj1", "proj2", "id")) %>%
  mutate(
...
provedit_similarity_matches <- read_csv("output/provedit/similarity_matches.csv") %>%
  separate(human_id, sep = "-", into = c("proj1", "proj2", "id")) %>%
  mutate(
    id = ifelse(nchar(id) == 1, paste0("0", id), id),
    human_id = ifelse(is.na(proj1), NA, paste0(proj1, "-", proj2, "-", id))
...
provedit_similarity <- read_csv("output/provedit/human_similarities.csv")
provedit_n_hu <- n_distinct(c(provedit_similarity$hu1, provedit_similarity$hu2))
provedit_max_similarity <- provedit_similarity %>%
  pull(similarity) %>%
  max()

provedit_bistro_matches <- read_csv("output/provedit/bistro_matches.csv") %>%
  mutate(match = str_to_sentence(match)) %>%
  mutate(incomplete = case_when(
    locus_count == 15 ~ "Complete profile",
    locus_count < 15 ~ "Incomplete profile"
  ))

provedit_thresh10_matches <- read_csv("output/provedit/bistro_log10LRs.csv") %>%
  mutate(
    match = ifelse(log10_lr >= 10 & !is.infinite(log10_lr), "Yes", "No"),
...
bm_similarity <- read_csv("output/bloodmeals/human_similarities.csv")
...
bm_similarity_matches <- read_csv("output/bloodmeals/similarity_matches.csv")

bm_bistro_matches_all <- read_csv("output/bloodmeals/bistro_matches.csv")
...
provedit_samp_composition <- provedit_exact_matches %>%
  ...
  mutate(
    ...
    bloodmeal_id_alt = ifelse(grepl("M", bloodmeal_id), paste0(first_part, props_new, second_part), bloodmeal_id)
  ) %>%
  ungroup() %>%
  select(-c(props_orig, props_new, first_part, second_part)) %>%
  separate(bloodmeal_id_alt,
    sep = "-",
    into = c(
      "project1", "project2",
      "contributor_id_dilution", "amplification_or_mixture",
      "experiment_or_mixture", "amplification_mixture",
      "experiment_mixture"
    ), remove = FALSE
  ) %>%
  separate(contributor_id_dilution,
    sep = "d",
    into = c("human_id", "dilution_condition")
  ) %>%
  mutate(
    project = paste0(project1, "-", project2),
    dilution_number = ifelse(is.na(dilution_condition),
      gsub("[a-z]|[A-Z]", "", experiment_or_mixture),
      gsub("[a-z]", "", dilution_condition)
    ),
    condition = ifelse(is.na(dilution_condition),
      gsub("M[0-9]", "", experiment_or_mixture),
      gsub("[0-9]", "", dilution_condition)
    ),
    experiment = ifelse(is.na(dilution_condition), experiment_mixture, experiment_or_mixture),
    template_mass = ifelse(is.na(dilution_condition), amplification_mixture, amplification_or_mixture),
    mixture_ratio = ifelse(amplification_or_mixture == template_mass, NA, amplification_or_mixture),
  ...
  select(-c( # duplicates
    project1, project2, dilution_condition, experiment_or_mixture, experiment_mixture,
    amplification_or_mixture, amplification_mixture, project,
    # unneeded
    dilution_number, condition, experiment, mixture_ratio
  )) %>%
  ungroup() %>%
...
```{r}
bistro_algorithm <- grViz("digraph bistro {
...
provedit_exact_matches_incorrect <- anti_join(provedit_exact_matches, provedit_samp_composition) %>%
  filter(!is.na(human_id) & match == "Yes")

provedit_similarity_matches_correct_none <- provedit_samp_composition %>%
  left_join(provedit_similarity_matches) %>%
  filter(!is.na(human_id)) %>%
  mutate(match = ifelse(is.na(match), "No", match))

provedit_similarity_matches_incorrect <- anti_join(provedit_similarity_matches, provedit_samp_composition) %>%
  filter(!is.na(human_id) & match == "Yes")

provedit_bistro_matches_correct_none <- provedit_samp_composition %>%
  left_join(provedit_bistro_matches %>%
    rename(bloodmeal_id = bloodmeal_id, human_id = human_id) %>%
    rowwise() %>%
...
provedit_bistro_matches_incorrect <- anti_join(provedit_bistro_matches %>%
  select(-c(locus_count, incomplete)) %>%
  rename(bloodmeal_id = bloodmeal_id, human_id = human_id) %>%
  rowwise() %>%
  mutate(human_id = ifelse(nchar(human_id) == 11, paste0(substr(human_id, 1, 10), 0, substr(human_id, 11, 11)), human_id)) %>%
  filter(match == "Yes"), provedit_samp_composition) %>%
  filter(!is.na(human_id))

provedit_thresh10_matches_correct_none <- provedit_samp_composition %>%
...
provedit_thresh10_matches_incorrect <- anti_join(provedit_thresh10_matches %>%
  rename(bloodmeal_id = bloodmeal_id, human_id = human_id) %>%
  rowwise() %>%
  mutate(human_id = ifelse(nchar(human_id) == 11, paste0(substr(human_id, 1, 10), 0, substr(human_id, 11, 11)), human_id)) %>%
  filter(match == "Yes"), provedit_samp_composition) %>%
...
```{r}
provedit_bistro_matches_by_samp <- provedit_bistro_matches_correct_none %>%
...
(provedit_match_plot <- provedit_bistro_matches_by_samp %>%
  ...
  ggplot(aes(fill = as.character(noc), x = as.character(value))) +
  geom_bar() +
...
provedit_bistro_matches_correct_none %>%
  group_by(incomplete) %>% # , multisource)
  mutate(tot = n()) %>%
  group_by(match, incomplete, tot) %>% # , multisource, tot) %>%
  tally() %>%
  filter(match == "Yes") %>%
  mutate(prop = n / tot)
```
...
```{r}
(provedit_incomplete <- provedit_bistro_matches_correct_none %>%
  mutate(incomplete = gsub(" profile", "", incomplete)) %>%
  ggplot(aes(x = incomplete, alpha = match)) +
  geom_bar(position = "fill", fill = "grey50", width = 0.5) +
  scale_alpha_manual(values = c(0, 1)) +
  labs(x = "Sample STR\nprofile type", y = "Proportion of matches\nidentified by bistro") +
...
(provedit_bistro_compare <- bind_rows(
  ...
  # pivot_longer(c(exact_match, similarity_match, bistro_match), names_to = 'algorithm', values_to = 'match') %>%
...
provedit_fig <- ggarrange(
  ggarrange(provedit_match_plot,
    provedit_incomplete,
    provedit_mixes,
    nrow = 1, labels = "AUTO", widths = c(1.2, 1, 1)
  ),
...
ggsave(plot = provedit_fig, filename = "figures/provedit.png", width = 9, height = 7)
```
